...
build_images () {
	...
	for DOCKER_TAG in "${DOCKER_TAGS[@]}"; do
		...
		if ! docker run --rm "bats-test-runner:$DOCKER_TAG" bash -c 'test ! -f ~/.ssh/*'; then
		...
		docker tag "bats-test-runner:$DOCKER_TAG" "kherring/bats-test-runner:$DOCKER_TAG"
	done
...
push_images () {
	...
	for DOCKER_TAG in "${DOCKER_TAGS[@]}"; do
		docker push "kherring/bats-test-runner:$DOCKER_TAG"
	done
